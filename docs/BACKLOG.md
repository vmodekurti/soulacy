...
| 5 | M5 Reliability & workboard depth | **12 → 13 → 14** | 12 ✅ 13 ✅ (catch-up hardening + GUI copy; artifact tracking: write_file detection from the run's tool trail → workboard_artifacts table → run.artifact events via E1 → task-modal Artifacts panel w/ download). 14 next: task collaboration primitives. |

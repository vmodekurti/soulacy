...
| 5 | M5 Reliability & workboard depth | **12 → 13 → 14** | 12 ✅ (latest-only catch-up hardened w/ multi-miss/corrupt-state/restart-sequence tests; catch_up surfaced in /schedule entries; Missed-runs column + copy in GUI; service-restart check on Mac/Docker still manual). 13 next: workboard artifact tracking (run.artifact via E1). |

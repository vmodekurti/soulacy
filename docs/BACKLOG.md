...
| 4 | M4 Voice | **10 → 11** | 10 ✅ spike done (docs/VOICE_SPIKE.md): sidecar bridge CONFIRMED, OpenAI Realtime (WebRTC + ephemeral keys) first, Gemini Live second; audio goes browser↔provider directly, sidecar is control-plane only; PoC scripts/poc-voice-sidecar.py passes E3 conformance (permanent test). 11 next: voice panel MVP per spike §3. |

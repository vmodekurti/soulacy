...
| 6 | M6 SDK & distribution | **E9 → E10 → E15 → E16 → E17 → E11 → E12 → E13** | E9 ✅ (sdk/ module: message/channel/llm/queue/vector/memory/storage contracts, stdlib-only, replace-directive wired; type aliases at all old paths so everything compiles unchanged; compat policy in sdk/README.md). E10 next: factory registries + decompose main.go. E15–E17 follow. |
